...
class SensorGreedy:
    ...
    def _fitness(self, coverage: float, inner_count: int) -> float:
        target = float(self.fitness_kwargs.get("target_coverage", 100.0))
        ...
        deficit = max(0.0, target - float(coverage))
        total_sensors = len(self.corner_positions) + int(inner_count)
        return float(min(float(coverage), target) - sensor_weight * total_sensors - deficit_penalty * deficit)
    ...
    def _select_best_candidate(
        ...
        if ndimage is None:
            return self._select_best_candidate_loop(covered, selected)
        ...
        best_gain = int(gain_map.max(initial=0))
        if best_gain <= 0:
            return None, 0, None

        y, x = np.unravel_index(int(np.argmax(gain_map)), gain_map.shape)
        point = (int(x), int(y))
        return point, best_gain, self._covered_indices(point)
    ...
    def _greedy_recursive(
        ...
        *,
        ...
        fitness = self._fitness(coverage, len(solution))
        ...
        point, gain, indices = self._select_best_candidate(covered, selected)
    ...
    def run(
        ...
        **_,
        ...
        self.best_fitness = self._fitness(self.best_coverage, len(result))
