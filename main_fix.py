...
class SensorDeployment:
    def __init__(self, map_name, coverage:int=45, generation:int=100):
        ...
        self.coverage = coverage         # 시각화 반경으로 사용 중인 값
        self.GEN = generation

        # MAP을 숫자형으로 강제 변환 (CSV 로딩 시 문자열(<U..) 방지)
    ...
    def inner_sensor_deploy(self, map, experiment_dir):
        ...
        inner_layer, inner_points, coverage_score = SensorGA(
            layer_inner, self.coverage, self.GEN, results_dir=experiment_dir
        ).run()
    ...
    def run(self):
        ...
        uncovered_score = int(np.sum(uncovered == 2))
        total_site = int(np.sum(self.MAP == 1))
...
if __name__ == "__main__":
    for i in range(1):
        ...
        instance.visual_module.showJetMap(title="Original Map", map_data=instance.MAP, filename="original_map")
        instance.run()
    for i in range(1):
    ...
    for i in range(1):
        instance = SensorDeployment(map_name="map_280x250.top", coverage=45, generation=100)
        instance.visual_module.showJetMap(title="Original Map", map_data=instance.MAP, filename="original_map")
        instance.run()
    for i in range(1):
        instance = SensorDeployment(map_name="map_280x250.mid", coverage=45, generation=100)
        instance.visual_module.showJetMap(title="Original Map", map_data=instance.MAP, filename="original_map")
        instance.run()
    for i in range(1):
        instance = SensorDeployment(map_name="map_280x250.bot", coverage=45, generation=100)
        instance.visual_module.showJetMap(title="Original Map", map_data=instance.MAP, filename="original_map")
        instance.run()
        
    for i in range(1):
        instance = SensorDeployment(map_name="map_1100x570.large", coverage=45, generation=100)
        instance.visual_module.showJetMap(title="Original Map", map_data=instance.MAP, filename="original_map")
        instance.run()
