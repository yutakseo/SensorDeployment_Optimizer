...
class SensorEvaluator:
    def __init__(self, map:np.ndarray, corner_points:list, sensor_points:list, coverage:int=45) -> None:
        ...
        self.base_map:np.ndarray
        self.sorted_sensors:list
        self.fitness_score:float
        self.uncovered_area:np.ndarray
         
    
    def _activation_map_(self):
        return self.model.forward(self.map)
    
    def rankSensors(self, MAP:np.ndarray, sensor_points:list, coverage:int=45):
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.model(MAP)
        with torch.no_grad():
            fitness_map = self.model(MAP)

        ranking = []
        shifted_sensor_points = np.array(sensor_points) - np.array(self.corners)
        ...
        self.sorted_sensors = ranking
        return ranking
        

    def fitnessFunc(self, MAP, sensor_list: list, coverage: int):
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    def extractUncovered(self, MAP, sensor_list: list, coverage: int):
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        tensor_map = torch.tensor(np.array(MAP), dtype=torch.float32, device=device)
        sensor = Sensor(MAP)
        # 모든 센서 배치
        ...
        self.uncovered_area = uncovered_positions
